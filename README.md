...
- **内容不含路径**：云端只存节点数据（tree.yaml + nodes/），不存本机路径；不同机器可以把同一个项目放在不同目录
...
节点类型是**自由字符串**，无硬性限制。`dt init` 时会自动创建一棵写作风格说明树（根节点「不同类型节点的写作风格要求」+ 四个子节点），模型通过 `dt tree` / `dt show` 渐进式读取即可理解每种类型的协作方式。

四种推荐写作风格：

| 类型 | 适合场景 | 协作方式 |
|------|---------|---------|
| `explore` | 需要推导的探索性问题 | 四层结构：已知前提 / 认知起点 / 推导逻辑 / 讨论共识 |
| `task` | 目标明确、需要执行的任务 | 任务描述 + 完成标准 + 执行记录 |
| `document` | 原始事实、外部资料、背景信息 | 只陈述不推导，保持原始性 |
| `decision` | 在多个选项之间做选择 | 选项对比 + 评估标准 + 结论 |
