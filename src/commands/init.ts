    ...
    .action((name: string) => {
      ...
      for (const node of STYLE_NODES) {
        const frontmatter: NodeFrontmatter = {
          ...
          edges: node.id === '001'
            ? [
                { target: '002', type: 'to', summary: '探索型节点写作风格' },
                { target: '003', type: 'to', summary: '任务型节点写作风格' },
                { target: '004', type: 'to', summary: '资源记录型节点写作风格' },
                { target: '005', type: 'to', summary: '决策型节点写作风格' },
              ]
            : [{ target: '001', type: 'from', summary: '节点类型说明' }],
          created: now,
        };
      ...
      console.log(chalk.dim(`  已创建 5 个写作风格说明节点（运行 \`dt tree\` 查看）`));
      console.log('');
    });
}
